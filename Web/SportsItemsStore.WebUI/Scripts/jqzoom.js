;(function($){
	$.fn.zoom = function(options){
		...
		var _option = {
		    align: "left",				//Current show the location of the picture, the enlarged picture in its relative position
		    thumb_image_width: 300,		// current show pictures of width
		    thumb_image_height: 400,	//current show pictures of the High
		    source_image_width: 1200,  	// zoom in to a picture of
		    source_image_height: 1600,	// zoom in to the picture of the High
		    zoom_area_width: 600, 		// zoom in to the picture of the display area of
		    zoom_area_height: "justify",// zoom in to the picture of the display area of the High
			zoom_area_distance: 10,     // 
			zoom_easing: true,          // not fade-in and fade-out
			click_to_zoom: false,
			zoom_element: "auto",
			show_descriptions: true,
			description_location: "bottom",
			description_opacity: 0.7,
			small_thumbs: 3,			// smaller pictures to show the quantity
			smallthumb_inactive_opacity: 0.4, 	// small picture is not active, the mask transparency
			smallthumb_hide_single: true,    	// 
			smallthumb_select_on_hover: false,
			smallthumbs_position: "bottom",		// small the location of the pictur
			show_icon: true,
			hide_cursor: false,			//mouse on the picture, is hide the pointer
			speed: 600,     			// 
			autoplay: true,				// whether to automatically play
			autoplay_interval: 6000, 	// Auto-play each picture of the dwell time
			keyboard: true,
			right_to_left: false,
		}
		...
		if($ul.is("ul") && $ul.children("li").length && $ul.find(".bzoom_big_image").length){
		  $ul.addClass('bzoom clearfix').show();
			...
			$li.add($bzoom_magnifier).mousemove(function(event){
			    ...
			    $bzoom_zoom_area.stop().fadeOut(400).animate({ 'opacity': 1 }, _option.speed * 0.7);
